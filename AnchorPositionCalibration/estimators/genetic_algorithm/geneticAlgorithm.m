function estimatedAnchors = geneticAlgorithm(distances_noisy, initialGuess, tagPos, bounds)
    ...
    options = optimoptions('ga', ...
        ...
        'PopulationSize', 200, ... % Adjusted population size
        'MaxGenerations', 500, ... % Increased number of generations
        'CrossoverFraction', 0.8, ... % Increased crossover fraction
        'MutationFcn', {@mutationadaptfeasible, 0.2}, ... % Adjusted mutation function
        'EliteCount', 5, ... % Increased number of elite individuals
        'InitialPopulationMatrix', generateInitialPopulation(initialGuess, numVars, 200, bounds), ... % Use initial guess to generate population
